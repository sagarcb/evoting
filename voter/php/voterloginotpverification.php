...
if (!empty($otp)) {
    if ($otp == $session_otp) {
        $sql = mysqli_query($conn, "SELECT * FROM voterlogin WHERE id = '{$unique_id}' AND otp = '{$otp}'");
        if (mysqli_num_rows($sql) > 0) {
            $null_otp = 0;
            $sql2 = mysqli_query($conn, "UPDATE voterlogin SET `email_verification_status` = '1',`otp`= '$null_otp' WHERE id = '{$unique_id}'");
            if ($sql2) {
